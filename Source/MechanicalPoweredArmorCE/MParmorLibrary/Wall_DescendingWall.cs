    ...
    protected virtual Command_Action Change()
    {
        var command_Action = new Command_Action
        {
            ...
            action = delegate
            {
                ...
                var map = Map;
                if (ThingMaker.MakeThing(
                        IsDescending
                            ? ThingDefOf.XFMParmor_DescendingWall
                            : ThingDefOf.XFMParmor_DescendingWall_Descending,
                        Stuff) is not Wall_DescendingWall wall_DescendingWall)
                {
                    return;
                }
                ...
                FleckMaker.ThrowDustPuff(
                    ...
                    new Vector3(Rand.Range(0.15f, -0.15f), 0f, Rand.Range(0.15f, -0.15f)),
                    map, 1.1f);
                FleckMaker.ThrowDustPuff(
                    position.ToVector3Shifted() +
                    new Vector3(Rand.Range(0.15f, -0.15f), 0f, Rand.Range(0.15f, -0.15f)),
