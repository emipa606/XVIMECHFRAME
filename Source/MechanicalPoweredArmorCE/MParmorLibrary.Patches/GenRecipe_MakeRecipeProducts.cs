...
[HarmonyPatch(typeof(GenRecipe), nameof(GenRecipe.MakeRecipeProducts), typeof(RecipeDef), typeof(Pawn),
    typeof(List<Thing>), typeof(Thing), typeof(IBillGiver), typeof(Precept_ThingStyle), typeof(ThingStyleDef),
    typeof(int?))]
public static class GenRecipe_MakeRecipeProducts
{
    private static bool Prefix(RecipeDef recipeDef, IBillGiver billGiver, Pawn worker, List<Thing> ingredients)
    {
        ...
        if (recipeDef.Worker is RecipeWorker_Maintain recipeWorker_Maintain && billGiver is MParmorBuilding building)
        {
